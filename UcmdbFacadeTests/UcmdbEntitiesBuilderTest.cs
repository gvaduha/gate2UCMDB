...
namespace UcmdbFacade.UnitTests
{
  ...
  class UcmdbEntitiesBuilderTest
  {
    ...
    public void BuilderInitWrongBuilderType()
    {
      ...
      ActualValueDelegate<object> code = () => new[] { typeof(TestType), typeof(Int32) }.Select(eb.AddTemplateClass).ToList();
      Assert.That(code, Throws.TypeOf<UcmdbFacadeException>());
    }
    ...
    public void CreateUnknownType()
    {
      ...
      ActualValueDelegate<object> code = () => eb.Create("cc_XXX");
      Assert.That(code, Throws.TypeOf<UcmdbFacadeException>());
    }
